...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { YolkenTest14cInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw YolkenTest14cInvalidDataException("`$name` is not set")
            is JsonNull -> throw YolkenTest14cInvalidDataException("`$name` is null")
            else -> throw YolkenTest14cInvalidDataException("`$name` is invalid, received $this")
        }
